/* 由 tcsb_grab.user.js 移植成 Chrome 外掛的 content script。
   注入範圍改由 manifest.json 的 content_scripts 決定。
   含：新版 onepage 結帳、免 cookie 瀏覽器加入、登出提醒（只認登入網址/可見密碼欄，避免誤報）。
   下單模式改用一般 fetch 問 /api/tcsb_mode。安全界線不變：不碰信用卡、不代登入。 */
  ...
  if (location.host === 'mfme.map.com.tw') {
    ...
    var mapIv = setInterval(function () {
        ...
        if (row) {
          ...
          for (var lv = 0; lv < 4 && node; lv++) {   // 自己 → 往上幾層都試點看看
            ...
            try {   // 這站是 ASP.NET，多半有 jQuery，用它觸發事件最準
              if (window.jQuery) window.jQuery(node).trigger('click');
            } catch (e) {}
      ...
      if (!inp || inp.getBoundingClientRect().width <= 0) {
        try { if (typeof window.openLightBox === 'function') window.openLightBox(1); } catch (e) {}
